...
export default authedSessionEventHandler(
    async ({ event, session, db }) => {
        ...
        await db
            ...
            .where(
                and(
                    eq(userFollows.userId, followerId),
                    eq(userFollows.followeeId, followeeData.id),
                ),
            )

        await Promise.all([
            invalidateUserContentCache(event, db, followerId, 'unfollow user'),
            invalidateUserContentCache(event, db, followeeData.id, 'unfollowed user'),
        ])
