...
- Follows and mutes use `user_follows` and `user_mutes`; privacy and notification preferences live in `user_settings`. Viewer relation reads are `no-store`. Public Setup lists remain cookie-independent; explicit `viewer=true`/`following=true` feeds apply the viewer's mutes. Only public new Setups notify followers. Push and webhook delivery are not implemented.
