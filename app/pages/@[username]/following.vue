...
const { data } = await useUserFollowees(username.value)
const followees = ref(data.value?.followees ?? [])
const pagination = ref(data.value?.pagination)

const loadMore = async () => {
    if (!pagination.value?.hasNext) return

    const next = await $fetch(`/api/users/${username.value}/followees`, {
        query: {
    ...
    followees.value = [...followees.value, ...next.followees]
    pagination.value = next.pagination
}

useSeo({
    title: data.value?.user.name
        ? t('user.follow.followingSeoTitle', { name: data.value.user.name })
        : undefined,
    ...
    >
        <div class="flex flex-col gap-3">
            <span class="sentence mx-3 text-sm">
                {{
                    $t('user.follow.followingDescription', {
                        count: pagination?.total ?? followees.length,
                    })
                }}
            </span>
